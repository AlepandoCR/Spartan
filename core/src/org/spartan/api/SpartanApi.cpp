    ...
    extern "C" {
         ...
         */
        SPARTAN_API_EXPORT int64_t spartan_test_vector_union(double* targetFuzzySet,
        ...
        SPARTAN_API_EXPORT int spartan_register_model(
            ...
            std::memcpy(&providedSignature,
                        static_cast<const uint8_t*>(opaqueHyperparameterConfig) + 60,
        ...
        /**
         * @brief Returns the number of scalar values exposed by the ProximalPolicyOptimization debug snapshot.
         *
         * @param agentIdentifier The target model identifier.
         * @return Scalar count on success, -1 if the model does not exist or is not ProximalPolicyOptimization.
         */
        SPARTAN_API_EXPORT int32_t spartan_proximal_policy_optimization_debug_get_scalar_count(
                const uint64_t agentIdentifier) {
            return engine.getProximalPolicyOptimizationDebugScalarCount(agentIdentifier);
        }

        /**
         * @brief Copies a scalar debug snapshot from a ProximalPolicyOptimization model into a Java-owned buffer.
         *
         * @param agentIdentifier  The target model identifier.
         * @param outputBuffer     Java-owned writable double buffer.
         * @param outputCount      Capacity (number of doubles) in @p outputBuffer.
         * @return Number of scalars written on success, -1 on error.
         */
        SPARTAN_API_EXPORT int32_t spartan_proximal_policy_optimization_debug_copy_scalars(
                const uint64_t agentIdentifier,
                double* outputBuffer,
                const int32_t outputCount) {
            if (outputBuffer == nullptr || outputCount <= 0) {
                SpartanEngine::logError("spartan_proximal_policy_optimization_debug_copy_scalars: invalid output buffer.");
                return -1;
            }

            return engine.copyProximalPolicyOptimizationDebugScalars(agentIdentifier, outputBuffer, outputCount);
        }
         ...
         * @param actionBuffer      Pointer to shared action buffer (Java-owned)
         * @param actionBufferSize  Total size of action buffer
         * @param actionFieldSize   Number of action fields per agent
